...
function DetailsService() {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:1337/api/careers/7?populate[post][populate]=*")
      .then((res) => {
          ...
          <div className="row">
            <div className="col-lg-12">
              ...
              {text2["attributes"]
                ...
                : "hgfhgf"}
              <h3 className="headdetail">Role: </h3>{" "}
              {text2["attributes"]
                ? text2["attributes"]["post"].map((x) => (
                    <h6 className="detail">{x.role}</h6>
                  ))
                : "hgfhgf"}
              <h3 className="headdetail">Industry Type: </h3>{" "}
              {text2["attributes"]
                ? text2["attributes"]["post"].map((x) => (
                    <h6 className="detail">{x.industries}</h6>
                  ))
                : "hgfhgf"}
              <h3 className="headdetail">Functional Area: </h3>{" "}
              {text2["attributes"]
                ? text2["attributes"]["post"].map((x) => (
                    <h6 className="detail">{x.functional}</h6>
                  ))
                : "hgfhgf"}
              <h3 className="headdetail">Employment Type: </h3>{" "}
              {text2["attributes"]
                ? text2["attributes"]["post"].map((x) => (
                    <h6 className="detail">{x.employement}</h6>
                  ))
                : "hgfhgf"}
              <h3 className="headdetail">Role Category: </h3>{" "}
              {text2["attributes"]
                ? text2["attributes"]["post"].map((x) => (
